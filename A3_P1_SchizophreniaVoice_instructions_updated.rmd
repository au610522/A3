...
# Should we remove the ones we do not know so we are sure to get all the information? 

# Removing the NA's (if any) from the features that we want to look into. 
danishData <- danishData[complete.cases(danishData[3:18]), ] # complete cases should contain columns 3 to 18. No cases are dropped.

# Pairing the participants by their matched control within study so that they are different between studies
danishData <- danishData %>% mutate(uPairID = paste(Participant, Study, sep = "_"), 
...
info1 <- danishData %>%  subset(Trial == 1) %>%
    group_by(Diagnosis) %>%
...
info3 <- danishData %>%  subset(Trial == 1) %>%
...
danishData$pause_dur <- (danishData$dur..s.- danishData$phonationtime..s.)
...
m1 <- lmer(data = danishData, iqr_s ~ 0 + Study + Study:Diagnosis + (1|uPairID)) 
# Model for proportion of spoken time 
m2 <- lmer(data = danishData, propSpoken_s ~ 0 + Study + Study:Diagnosis + (1|uPairID))
# Model for speech rate
m3 <- lmer(data = danishData, speechrate_s ~ 0 + Study + Study:Diagnosis + (1|uPairID))
# Model for duration of pause
m4 <- lmer(data = danishData, pause_dur_s ~ 0 + Study + Study:Diagnosis + (1|uPairID))
