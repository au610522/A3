...
# at last we can remove NA's - removes a lot of the studies because they do not have all the information 
...
data$Diagnosis[data$Diagnosis == '1'] <- 'Schizophrenia'
...
# Removing the NA's (if any) from the features that we want to look into. 
#data <- data[complete.cases(data[3:19]), ] # complete cases should contain columns 3 to 19. 8 observations are dropped.

# Counted by diagnosis for 1 trial
info1 <- data %>%  subset(Trial == 1) %>%
    group_by(Diagnosis) %>%
...
```{r}
# Making new column for pause duration
data$pause_dur <- 0 # Assigning "0" to all rows

# For loop to assign pause duration for all rows
for (i in data$uID) {
    if (data$npause[i] == 0) { # If the number of pauses are 0 we assign 0.
        data$pause_dur[i] <- 0 }
    else { # If not, we assign the average duration of the pauses
        data$pause_dur[i] <- round((data$dur..s.[i] - data$phonationtime..s.[i]) / (data$npause[i]), 2) }}


# Only taking the first 4 studies
danishData <- subset(data, Study < 5)

# Scaling the features
danishData$iqr_s <- scale(danishData$iqr) # Representative of pitch variability
danishData$propSpoken_s <- scale(danishData$phonationtime..s.) # Proportion of spoken time
danishData$speechrate_s <- scale(danishData$speechrate..nsyll.dur.) # Speech rate
danishData$pause_dur_s <- scale(danishData$pause_dur) # Duration of pause

# Building the models for the different features
# Model for inter quantile range
m1 <- lmer(data = danishData, iqr_s ~ 1 + Diagnosis + (1|uID))
# Model for proportion of spoken time 
m2 <- lmer(data = danishData, propSpoken_s ~ 1 + Diagnosis + (1|uID))
# Model for speech rate
m3 <- lmer(data = danishData, speechrate_s ~ 1 + Diagnosis + (1|uID))
# Model for duration of pause
m4 <- lmer(data = danishData, pause_dur_s ~ 1 + Diagnosis + (1|uID))


# Only taking study 5 and 6
chineseData <- subset(data, Study == 5 | Study == 6)

for (i in chineseData$uID) {
    if (chineseData$npause[i] == 0) { # If the number of pauses are 0 we assign 0.
        chineseData$pause_dur[i] <- 0 }
    else { # If not, we assign the average duration of the pauses
        chineseData$pause_dur[i] <- round((chineseData$dur..s.[i] - chineseData$phonationtime..s.[i]) / (chineseData$npause[i]), 2) }}
# Scaling the features
chineseData$iqr_s <- scale(chineseData$iqr) # Representative of pitch variability
chineseData$propSpoken_s <- scale(chineseData$phonationtime..s.) # Proportion of spoken time
chineseData$speechrate_s <- scale(chineseData$speechrate..nsyll.dur.) # Speech rate
chineseData$pause_dur_s <- scale(chineseData$pause_dur) # Duration of pause

# Building the models for the different features
# Model for inter quantile range
m5 <- lmer(data = chineseData, iqr_s ~ 1 + Diagnosis + (1|uID)) 
# Model for proportion of spoken time 
m6 <- lmer(data = chineseData, propSpoken_s ~ 1 + Diagnosis + (1|uID))
# Model for speech rate
m7 <- lmer(data = chineseData, speechrate_s ~ 1 + Diagnosis + (1|uID))
# Model for duration of pause
m8 <- lmer(data = chineseData, pause_dur_s ~ 1 + Diagnosis + (1|uID))


# Only taking study 7
japaneseData <- subset(data, Study > 6)

# Scaling the features
japaneseData$iqr_s <- scale(japaneseData$iqr) # Representative of pitch variability
japaneseData$propSpoken_s <- scale(japaneseData$phonationtime..s.) # Proportion of spoken time
japaneseData$speechrate_s <- scale(japaneseData$speechrate..nsyll.dur.) # Speech rate
japaneseData$pause_dur_s <- scale(japaneseData$pause_dur) # Duration of pause

# Building the models for the different features
# Model for inter quantile range
m9 <- lmer(data = japaneseData, iqr_s ~ 1 + Diagnosis + (1|uID)) 
# Model for proportion of spoken time 
m10 <- lmer(data = japaneseData, propSpoken_s ~ 1 + Diagnosis + (1|uID))
# Model for speech rate
m11 <- lmer(data = japaneseData, speechrate_s ~ 1 + Diagnosis + (1|uID))
# Model for duration of pause
m12 <- lmer(data = japaneseData, pause_dur_s ~ 1 + Diagnosis + (1|uID))
