...
# Get the model accuracy and kappa values
metrics(log_reg_results, truth = Diagnosis, estimate = pred_class)

# Confusion matrix
conf_mat(log_reg_results, truth = "Diagnosis", estimate = "pred_class")

# Calculate sensitivity and specificty
caret::sensitivity(log_reg_results$Diagnosis, log_reg_results$pred_class) # 0.86
caret::specificity(log_reg_results$Diagnosis, log_reg_results$pred_class) # 0.41
...
for (i in 1:max(data$fold)){
  ...
  # Clean environemnt
  rm(cv_train, cv_test, cv_log_reg)
}
...
# Get the model accuracy and kappa values
metrics(cv_results, truth = Diagnosis, estimate = pred_class)

# Confusion matrix
conf_mat(cv_results, truth = "Diagnosis", estimate = "pred_class")

# Calculate sensitivity and specificty
caret::sensitivity(cv_results$Diagnosis, cv_results$pred_class) # 0.57
caret::specificity(cv_results$Diagnosis, cv_results$pred_class) # 0.53
